...
## Overview
This document summarizes the implementation of event emission functionality and comprehensive tests for the Astre smart contract.
...
### Run all event tests:
```bash
cargo test --package astre_contract -- test_event
```

### Run specific event test:
```bash
cargo test --package astre_contract -- test_event_pool_creation_emits_correct_event
```

### Run with output:
```bash
cargo test --package astre_contract -- test_event --nocapture
```
